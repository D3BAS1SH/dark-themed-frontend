...
export const Card = (props:ComponentPropsWithoutRef<'div'>&{ color?:string; buttonText?:string}) => {
    const {className,color,children,buttonText} = props;
    return <div className={twMerge("relative z-0 p-8 md:p-10 group",className)}>
    ...
    <div className={twMerge('absolute size-16 rounded-xl bg-fuchsia-500 group-hover:bg-fuchsia-500 transition duration-300 top-1.5 right-1.5 -z-10',color==='lime' && 'bg-lime-500 group-hover:bg-lime-400',color==='cyan' && 'bg-cyan-500 group-hover:bg-cyan-400',color==='violet' && 'bg-violet-500 group-hover:bg-violet-400')}></div>
